...
pos_hole_1 = 0.5    #center of the hole position referred to the center of the channel, near the inlet
pos_hole_2 = 0.25    #center of the hole position referred to the center of the channel, near the outlet
...
control_parameter_3 = 0.7*ch_w #control parameter: max position of the point (center of the hole + diameter of the hole)
control_parameter_4 = 0.15*ch_w #control parameter: min position of the point (center of the hole - diameter of the hole)
...
if r_w/2+ch_w*pos_hole_1+h_d/2 >= r_w/2+control_parameter_1:
  pos_hole_1 = 0.9*control_parameter_1/ch_w
  h_d = control_parameter_2
  print("Error: Excessive hole diameter. New hole diameter set by default")
elif ch_w*pos_hole_1 >= control_parameter_1*0.95:
  pos_hole_1= 0.9*control_parameter_1/ch_w
  print("Error: Hole position near limit. New hole center and hole diameter set by default")


if r_w/2+ch_w*pos_hole_1-h_d/2 <= r_w/2+control_parameter_2:
  pos_hole_1 = 2*control_parameter_2/ch_w
  h_d = control_parameter_2
  print("Error: Excessive hole diameter or hole position. New hole diameter and hole center set by default")
elif ch_w*pos_hole_1 <= control_parameter_2:
  pos_hole_1 = control_parameter_2*2
  print("Error: Hole position near limit. New hole center set by default")

if r_w/2+ch_w*pos_hole_2+h_d/2 >= r_w/2+control_parameter_3:
  pos_hole_2 = 0.9*control_parameter_3/ch_w
  h_d = control_parameter_4
  print("Error: Excessive hole diameter. New hole diameter set by default")
elif ch_w*pos_hole_2 >= control_parameter_3*0.95:
  pos_hole_2= 0.9*control_parameter_3/ch_w
  print("Error: Hole position near limit. New hole center and hole diameter set by default")


if r_w/2+ch_w*pos_hole_2-h_d/2 <= r_w/2+control_parameter_4:
  pos_hole_2 = 2*control_parameter_4/ch_w
  h_d = control_parameter_4
  print("Error: Excessive hole diameter or hole position. New hole diameter and hole center set by default")
elif ch_w*pos_hole_2 <= control_parameter_4:
  pos_hole_2 = control_parameter_4*2
  print("Error: Hole position near limit. New hole center set by default")
...
for k in range(0, 4, 1):
  ...
  a.addPoint(-(ch_w*pos_hole_1 + h_d) - r_w/2,              0, z[k], el_s, 3+k*100)
  a.addPoint(-(ch_w*pos_hole_1 - h_d) - r_w/2,              0, z[k], el_s, 4+k*100)
  a.addPoint(r_w/2,                                       0, z[k], el_s, 6+k*100)
  a.addPoint((ch_w*pos_hole_2 + h_d) + r_w/2,               0, z[k], el_s, 8+k*100)
  a.addPoint((ch_w*pos_hole_2 - h_d) + r_w/2,               0, z[k], el_s, 7+k*100)
...
for i in range(11, 20, 1):
  for j in range(0, len(z), 1):
    if i == 19:
      ...
      gmsh.model.mesh.setTransfiniteCurve(abs(i + j*100), 11, meshType="Bump", coef=-0.20)
    elif i == 15:
...
for i in range(0, len(outletblock), 1):
  if i >= ((len(outletblock)) - ((len(z)) - 1)) and i%3 == 0:
  ...
  elif i >= ((len(outletblock)) - ((len(z)) - 1)) and i%3 != 0:
  ...
  elif ((len(outletblock)) - ((len(z)) - 1)) > i >= ((len(outletblock)) - 2*((len(z)) - 1)) and i%3 != 0:
        gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, outletblock[i][0][1])], oriented=True)[2][1], 11, meshType="Bump", coef=-0.20)
  elif ((len(outletblock)) - ((len(z)) - 1)) > i >= ((len(outletblock)) - 2*((len(z)) - 1)) and i%3 == 0:
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, outletblock[i][0][1])], oriented=True)[1][1], 11, meshType="Bump", coef=-0.20)
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, outletblock[i][0][1])], oriented=True)[3][1], 11, meshType="Bump", coef=0.20)
  elif i%3 == 0:
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, outletblock[i][0][1])], oriented=True)[1][1], 11, meshType="Progression", coef=0.80)
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, outletblock[i][0][1])], oriented=True)[3][1], 11, meshType="Progression", coef=-0.80)
  else:
...
for i in range(0, len(electrodeblock), 1):
  ...
  elif i >= (len(electrodeblock) - (len(z) - 1)) and i%3 != 0: #vale el mismo algoritmo para el resto de los electrodeblock, con la posicion "i" adecuada.
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, electrodeblock[i][0][1])], oriented=True)[2][1], 11, meshType="Bump", coef=0.20)
  elif i >= (len(electrodeblock) - (len(z) - 1)) and i%3 == 0:
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, electrodeblock[i][0][1])], oriented=True)[1][1], 11, meshType="Bump", coef=0.20)
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, electrodeblock[i][0][1])], oriented=True)[3][1], 11, meshType="Bump", coef=-0.20)
...
for i in range(0, len(downoutletblock), 1):
  if i >= ((len(downoutletblock)) - ((len(z)) - 1)) and i%3 == 0:
  ...
  elif i >= ((len(downoutletblock)) - ((len(z)) - 1)) and i%3 != 0:
  ...
  elif ((len(downoutletblock)) - ((len(z)) - 1)) > i >= ((len(downoutletblock)) - 2*((len(z)) - 1)) and i%3 != 0:
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][0][1])], oriented=True)[2][1], 11, meshType="Bump", coef=-0.20)
  elif ((len(downoutletblock)) - ((len(z)) - 1)) > i >= ((len(downoutletblock)) - 2*((len(z)) - 1)) and i%3 == 0:
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][0][1])], oriented=True)[1][1], 11, meshType="Bump", coef=-0.20)
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][0][1])], oriented=True)[3][1], 11, meshType="Bump", coef=0.20)  
  elif i%3 == 0:
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][0][1])], oriented=True)[1][1], 11, meshType="Progression", coef=0.80)
    gmsh.model.mesh.setTransfiniteCurve(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][0][1])], oriented=True)[3][1], 11, meshType="Progression", coef=-0.80)
  else:
...
print(downoutletblock)

#TRANSFINITE SURFACES, VOLUMES AND RECOMBINATION

for i in range(0, len(downoutletblock), 1):
  print(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][0][1])], oriented=True))

for i in range(0, len(outletblock), 1):
      print(gmsh.model.getBoundary(dimTags=[(2, outletblock[i][0][1])], oriented=True))

#for i in range(0, len(downoutletblock), 1):
      #print(gmsh.model.getBoundary(dimTags=[(2, downoutletblock[i][4][1])], oriented=True))
...
gmsh.write("vrfb_tailoring_ft_h_3d_20230329.msh")
